# Workspace Session Terminals

一个 VS Code 侧边栏扩展，用三个原生 TreeView 管理：

![Workspace Session Terminals 侧边栏界面](media/base-ui.png)
...
- Zellij 管理：列出 zellij 会话，支持新建、附加、重命名、结束、删除和自动刷新。
...
- `workspaceSessionTerminals.autoSaveIntervalMs`: 默认 `3000`，已记住会话终端的状态保存间隔。
- `workspaceSessionTerminals.autoRefreshIntervalMs`: 默认 `3000`，tmux/zellij TreeView 自动刷新间隔。
- `workspaceSessionTerminals.logToOutput`: 默认 `true`，把诊断 JSONL 同步到 Output Channel。
...
测试会打开 Workspace Session Terminals 侧边栏，验证三个 TreeView 的 UI 可见性，读取扩展运行状态，并在环境安装了 `tmux`/`zellij` 时创建和清理真实会话。
